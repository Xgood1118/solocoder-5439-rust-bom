// Custom R1 feishu write for 5439-rust-bom
...
const COMMIT_SHA = 'd006746abf02358ec83ae1b1d86cd40894bd3540';
...
(async () => {
  ...
  const dissatText = fs.readFileSync(`SoloCoder/${FOLDER}/tmp/5439-r1-dissat.txt`, 'utf8').trim();
  ...
  const filter = JSON.stringify({
    filter: {
      ...
      conditions: [
        ...
        { field_name: "轮次", operator: "is", value: "第一轮" }
      ]
    }
  });
  ...
  if (searchRes.code === 0 && searchRes.data && searchRes.data.items && searchRes.data.items.length > 0) {
    ...
    action = 'update';
    console.log('Found existing R1 record, will UPDATE:', recordId);
  } else {
    console.log('No R1 record found, creating new...');
  }
  ...
  const payloadFile = `SoloCoder/${FOLDER}/tmp/_5439-r1-feishu-payload.json`;
